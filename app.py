...
# 🧠 [오렌즈 경쟁 입지 비교 기능 통합] 정밀 상권 컨설팅 UI 함수
def render_location_consulting_ui(store_name, store_type, address):
    addr_str = str(address)
    
    # 📌 오렌즈(O-LENS) 근접 가능성 자동 판단 (주요 상권 키워드 포함 여부 기준)
    has_olens_nearby = any(k in addr_str or k in store_name for k in ['역', '대학', '대로', '지하', '광장', '로', '동', '시청', '구청'])
    
    if has_olens_nearby:
        olens_comp_text = """
        * **주변 경쟁사 현황:** 반경 300m~500m 이내 주요 경쟁 브랜드 **'오렌즈(O-LENS)'** 입지 가능성이 매우 높은 **과열 경쟁 상권**입니다.
        * **입지적 특성 비교:** 오렌즈가 지상 전면 메인 동선 노출에 주력한다면, 본 지점은 **접근 편의성 및 고정비(임대료) 대비 매출 효율성**에 강점이 있습니다.
        * **경쟁 대응 컨설팅:** 
          1. 가격 직접 할인전보다는 **렌즈미 독점 PB 라인업(악마원데이, 트루핏 등)**을 전면에 연출하여 상품 차별화를 선점하세요.
          2. **시력 검안 연계 및 난시용/기능성 렌즈 세트 제안**으로 오렌즈 대비 평균 객단가를 상향시키는 전략이 유효합니다.
        """
    else:
        olens_comp_text = """
        * **주변 경쟁사 현황:** 반경 500m 이내 경쟁 브랜드(오렌즈) 영향권에서 벗어난 **독점/지역 선점 상권**입니다.
        * **입지적 특성 비교:** 경쟁점 유입으로 인한 고객 이탈 리스크가 낮아 지역 내 미용/컬러렌즈 수요를 독식할 수 있는 입지적 기회가 존재합니다.
        * **경쟁 대응 컨설팅:** 
          1. 1020 영타겟 타깃 **1만~2만원대 가성비 PB 라인업**과 SNS 프로모션을 적극 활용해 상권 내 점유율을 빠르게 고착화하세요.
          2. 지역 단골 고객 대상 **정기 교체 알림 CRM 및 멤버십 혜택**을 강화하여 고객 고정화에 집중하세요.
        """
    ...
    if any(k in addr_str for k in ['지하', '지하상가', '역사', '역내']):
        ...
        details = {
            "유동성·접근성": """
            * **입지 특성 및 보행 동선:** 지하철 출입구 및 지하상가 메인 통로에 접해 있어 출퇴근시간대(08~09시, 18~20시) 유동인구 집약도가 매우 우수합니다.
            * **가시성 및 시인성:** 보행 동선 정면에 위치하여 접근성은 뛰어나나, 지하 구조의 한계로 인해 지상 전면 노출 및 간판 시인성은 제한됩니다.
            * **핵심 개선 포인트:** 입구 전면부에 조명형 렌즈 픽업 진열대를 설치하여 유동객의 시선을 즉각적으로 사로잡는 전략이 유효합니다.
            """,
            "배후 수요성": """
            * **배후 거주 세대:** 반경 500m 내 주거용 아파트 세대수보다 통행 유동인구 비중이 압도적인 이동형 상권입니다.
            * **소비 타깃층 분석:** 2030 직장인 및 통학 학생 비중이 70% 이상을 차지하며, 정기 방문보다는 동선 상의 충동 구매 성향이 강합니다.
            * **객단가 특성:** 단품 중심의 빠른 소비가 일어나며, 1만~2만원대 트렌디 컬러렌즈 및 원데이 소포장 제품 선호도가 높습니다.
            """,
            "집객력·활성화": """
            * **매출 활성화도:** 단위 면적당 발자국(Foot Traffic)이 매우 높으며, 입구 진열 상태에 따른 결제 전환율 변동 폭이 큽니다.
            * **시간대별 소비 집중도:** 퇴근 시간대(17시~20시)에 일 매출의 45% 이상이 집중되는 뚜렷한 피크 타임 특성을 보입니다.
            * **집객 전략:** 원데이 렌즈 할인 행사 및 타임 세일 입간판을 전면에 노출하여 결제 전환을 극대화해야 합니다.
            """,
            "경쟁성·희소성": f"""
            * **경쟁점 밀집도:** 동일 지하상가 구역 내 브랜드 안경원 및 저가형 렌즈 샵이 밀집되어 있어 가격 경쟁 압박이 존재합니다.
            {olens_comp_text}
            """,
            "입지 안정성": """
            * **시설 및 계약 리스크:** 역사/지하상가 시설 관리 주체의 재계약 조건 및 높은 점포 임대료가 고정비 부담으로 작용할 수 있습니다.
            * **손익 관리(BEP):** 고정 임대료를 상쇄할 수 있도록 원가율이 낮고 마진율이 높은 PB 컬러렌즈 판매 비중을 40% 이상으로 유지해야 합니다.
            """
        }
        ...
        details = {
            "유동성·접근성": """
            * **입지 특성 및 보행 동선:** 메인 대로변 입지로 지하철역 출입구 및 버스 정류장 접면성이 최상위 수준입니다.
            * **가시성 및 전면 노출:** 전면 파사드 간판 노출도가 뛰어납니다.
            * **접근 편의성:** 대중교통 이용객의 접근이 용이하여 외부 유입 고객 비중이 극대화되는 초우수 입지입니다.
            """,
            "배후 수요성": """
            * **배후 수요 융합도:** 인근 오피스 상업시설 직장인과 주말 외부 유입 인구가 복합적으로 형성되어 있습니다.
            * **소비 연령대:** 20대부터 40대까지 다양하게 분포하여 미용 렌즈뿐만 아니라 기능성(난시/프리미엄) 렌즈 수요층도 풍부합니다.
            * **구매력 특성:** 평균 객단가가 높게 형성될 수 있는 잠재력을 갖추고 있어 세트 판매 전략이 용이합니다.
            """,
            "집객력·활성화": """
            * **주중/주말 매출 균형:** 주중 직장인 퇴근 수요와 주말 약속/데이트 방문객 수요가 균형을 이루어 매출 기복이 적습니다.
            * **체류 시간 및 전환율:** 매장 유입 인구가 많으며 체험존 구성 시 브랜드 몰입도가 극대화됩니다.
            * **집객 전략:** 신상품 체험 갤러리 탭과 전면 디스플레이를 활용해 유입 유효 고객을 적극 확대해야 합니다.
            """,
            "경쟁성·희소성": f"""
            * **경쟁 과열 수준:** 반경 300m 내 대형 브랜드 안경원 및 렌즈 전문점이 다수 위치하여 상시 경쟁 상태입니다.
            {olens_comp_text}
            """,
            "입지 안정성": """
            * **상권 수명 및 지속성:** 유동인구가 보장된 성숙 상권으로 경기 변동에 대한 탄력성이 뛰어납니다.
            * **손익 관리:** 높은 임대료 비중을 극대화된 매출로 극복할 수 있도록 마진율 중심의 품목 믹스(PB+글로벌)가 필수적입니다.
            """
        }
        ...
        details = {
            "유동성·접근성": """
            * **보행 동선 특성:** 대학가 메인 상권 로드샵 및 등하교 핵심 동선에 위치하여 청년층 보행 유동이 집중됩니다.
            * **가시성:** 트렌디한 인테리어와 전면 개방형 매대 구성 시 학생들의 매장 유입 효과가 비약적으로 증가합니다.
            """,
            "배후 수요성": """
            * **타깃 인구 밀도:** 1020세대 대학생 및 인근 자취생 밀도가 압도적으로 높아 컬러렌즈 핵심 소비층이 형성되어 있습니다.
            * **트렌드 민감도:** SNS 이슈 및 연예인/인플루언서 착용 렌즈에 대한 반응 속도가 매우 빠르고 민감합니다.
            * **가격대 선호도:** 1만~2만원대 가성비 컬러렌즈 및 1+1 행사 제품 구매 비중이 전체의 60% 이상을 차지합니다.
            """,
            "집객력·활성화": """
            * **학기 중 집객 폭발력:** 개강 시즌(3~6월, 9~12월) 동안 유동인구와 결제 건수가 폭발적으로 증가합니다.
            * **비수기 변동성:** 여름/겨울 방학 기간(연 약 4개월) 동안 유동인구가 감소하는 계절적 특성을 보입니다.
            * **대응 전략:** 학기 중 단골 고객 DB를 확보하여 방학 기간 택배 배송 및 SNS 쿠폰 이벤트를 진행해야 합니다.
            """,
            "경쟁성·희소성": f"""
            * **시장 트렌드 경쟁:** 유사 저가 브랜드와의 트렌드 경쟁이 치열합니다.
            {olens_comp_text}
            """,
            "입지 안정성": """
            * **트렌드 주기 리스크:** 유행 주기가 빠른 영타겟 상권 특성상 매장 디스플레이 및 연출의 주기적 갱신이 필요합니다.
            * **연간 안정성:** 방학 비수기 기간의 고정비 관리를 위해 방학 전 세트 판매로 매출 축적액을 확보해야 합니다.
            """
        }
    else:
        ...
        details = {
            "유동성·접근성": """
            * **보행 및 차량 접근성:** 빠른 통행성 유동인구보다는 거주민의 목적형 정체 유동이 주를 이루며, 도보 및 자차 접근성이 안정적입니다.
            * **동선 접면성:** 아파트 단지 입구, 대형 마트, 학원가, 행정복지센터 등 생활 밀착형 동선에 입지해 있습니다.
            """,
            "배후 수요성": """
            * **배후 거주 세대 밀도:** 반경 500m 내 대단지 아파트 및 주택가가 밀집되어 있어 거주민 고정 고객층이 매우 견고합니다.
            * **소비 주체 및 객단가:** 초중고 학생 자녀를 둔 학부모 및 3040 거주민 비중이 높아 구매력이 일정하고 안정적입니다.
            * **선호 제품군:** 미용 컬러렌즈와 함께 원데이 투명렌즈, 난시용(토릭) 렌즈, 고단가 프리미엄 제품군 수요가 높습니다.
            """,
            "집객력·활성화": """
            * **재방문 목적 구매:** 즉흥 구매 비중은 낮으나 시력 검안 및 정기 교체를 위한 단골 재방문 주기가 매우 일정합니다.
            * **고객 충성도:** 한 번 확보한 고객의 타 매장 이탈률이 적어 고객 관계 관리(CRM) 시 매출 유지가 용이합니다.
            * **객단가 증대 전략:** 난시 전문 검안 서비스 및 대용량 원데이 렌즈 패키지 제안으로 평균 객단가를 높여야 합니다.
            """,
            "경쟁성·희소성": f"""
            * **상권 내 독점력:** 동네 밀착형 중심 상권으로 추가적인 대형 브랜드 진입 가능성이 낮아 점유율 유지가 쉬운 블루오션 특성을 띱니다.
            {olens_comp_text}
            """,
            "입지 안정성": """
            * **경기 탄력성 및 방어력:** 경기 변동에 영향을 적게 받는 배후 주거 상권으로 매출 하락 리스크가 매우 낮습니다.
            * **장기 점포 운영:** 임대료 변동 폭이 안정적이며, 장기적이고 예측 가능한 고정 수익을 창출하기에 가장 최적화된 입지입니다.
            """
        }
    ...
    st.markdown("<p style='font-size:13px; font-weight:bold; color:#334155; margin-bottom:8px;'>👇 아래 항목 버튼을 누르시면 오렌즈(O-LENS) 입지 비교를 포함한 세부 정밀 평가 사유와 솔루션을 확인하실 수 있습니다.</p>", unsafe_allow_html=True)
    ...
    # 순수 마크다운 방식으로 깔끔하게 노출되는 상세 카드
    active_item = st.session_state[session_key]
    st.markdown(f"#### 📌 [{active_item}] 상세 정밀 평가 사유 & 솔루션 (점수: {scores[active_item]})")
    st.markdown(details[active_item])

    st.markdown("---")
    
    # 하단 컨설팅 종합 가이드 (순수 마크다운 적용)
