...
def render_location_consulting_ui(store_name, store_type, address):
    ...
    if olens_case in ["과열_지하", "과열_대로", "경쟁_역세권"]:
        olens_comment = (
            f"• <b>상권 입지 비교:</b> 오렌즈가 대로변 메인 동선의 시인성을 선점한 구역이나, **{store_name}** 매장은 **{store_type}** 형태를 활용해 임대료 대비 실속형 접근성과 도보 밀착 동선을 확보했습니다.\n"
            "• <b>핵심 대응 전략:</b> 소모성 가격전 대신 렌즈미 단독 PB(악마원데이/트루핏) 전면 진열 및 정밀 검안 연계 세트 판매로 결제 객단가를 차별화하세요."
        )
    elif olens_case == "경쟁_대학가":
        olens_comment = (
            f"• <b>상권 입지 비교:</b> 대학가 내 오렌즈와 영타겟 유입 경쟁이 유발되는 구역이나, **{store_name}** 매장은 1020 선호 가성비 품목 믹스와 학생 동선 접근성에 입지적 강점이 있습니다.\n"
            "• <b>핵심 대응 전략:</b> 인스타그램 이슈 렌즈 즉시 스팟 진열 및 학생증 제휴 할인을 적용해 개강 시즌 유효 고객을 적극 선점하세요."
        )
    elif olens_case in ["독점_주거", "독점_학원가"]:
        olens_comment = (
            f"• <b>상권 입지 비교:</b> 오렌즈 중심의 대형 로드샵 상권과 차별화되는 **{location_type}** 입지로, 반경 500m 내 경쟁사 영향권에서 벗어난 **고정 단골 독점 상권**입니다.\n"
            "• <b>핵심 대응 전략:</b> 거주민/학생 대상 정기 교체 알림 CRM 문자 발송 및 난시용/원데이 대용량 패키지 세일즈로 재방문율을 높이세요."
        )
    else:
        olens_comment = (
            f"• <b>상권 입지 비교:</b> 경쟁 브랜드(오렌즈)의 직접적 영향력이 적은 블루오션 입지로, **{store_name}** 매장의 지역 밀착형 영업이 대단히 유리한 환경입니다.\n"
            "• <b>핵심 대응 전략:</b> 1만~2만원대 가성비 PB 라인업 전면배치와 지역 커뮤니티 마케팅을 적용해 지역 내 점유율을 고착화하세요."
        )

    # 📌 6. 초디테일 진단 및 실행 솔루션 텍스트 (깊이감 보완)
    details = {
        "유동성·접근성": f"""
        * **보행 동선 및 인프라 진단:** **{store_name}** 매장은 **{location_type}** 중심 보행축에 입지해 있습니다. 통행 유동객의 유효 접면률은 약 **{s_flow * 3.8:.1f}%** 수준으로 측정되었으며, 대중교통 이용객 및 도보 유입 동선 상의 접근 용이성이 진단 점수 **{s_flow}점 / 25점**의 근거가 됩니다.
        * **시인성 및 간판 파사드 노출:** 전면 도로 폭 및 보행 방향에서의 인지 시차(Eye-contact Time)를 분석한 결과, 주 보행 동선 상에서 고객이 매장을 발견하고 진입 판단을 내리기까지의 피크 시인성이 양호한 상태입니다.
        * **구체적 VMD 개선 솔루션:** 
          1. **시선 유도형 픽업 존:** 매장 입구 1m 내에 고휘도 조명이 설치된 메인 픽업 매대를 신설하여 유동 인구의 즉흥 유입율을 높이세요.
          2. **파사드 연출 강화:** 야간 및 유동 집중 시간대에 맞춰 매장 전면 조명 럭스(Lux)를 높이고 시즌 메인 렌즈 포스터를 프레임 정면에 노출하세요.
        """,
        "배후 수요성": f"""
        * **세대수 및 타깃 인구 분석:** 반경 500m 내 상주 거주 인구와 주중/주말 유입 타깃의 연령대 구성을 분석하여 **{s_demand}점 / 25점**을 산출했습니다. 핵심 구매층인 1020 영타겟과 3040 거주/직장인 비중이 약 **{s_demand * 3.9:.0f}%**의 안정적 소비 비율을 형성하고 있습니다.
        * **소비 파워 및 구매력 특성:** 가격 민감도가 높은 컬러렌즈 선호층과 고단가 원데이 투명/난시용 렌즈를 선호하는 목적형 구매층이 혼재된 상권 특성을 보입니다.
        * **맞춤형 상품 믹스(MD) 전략:**
          1. **입구부:** 1만~2만원대 트렌디 가성비 컬러렌즈 및 1+1 기획전 매대를 배치하여 영타겟 유입을 극대화하세요.
          2. **카운터/검안부:** 원데이 투명 대용량 팩 및 난시(토릭) 렌즈 패키지를 세트 구성하여 평균 결제 객단가를 상향시키세요.
        """,
        "집객력·활성화": f"""
        * **발자국(Foot Traffic) 및 CVR:** 유동인구 대비 실제 매장 유입 고객 비중 및 결제 전환율(CVR)을 정밀 산정한 결과 **{s_traffic}점 / 20점**의 활성화 수준을 보입니다.
        * **시간대별 매출 피크 타임:** 상권 특성상 메인 피크 타임대에 일 전체 매출의 약 **{min(75, s_traffic * 3.3):.0f}%**가 집중 발생되는 뚜렷한 골든 타임대가 존재합니다.
        * **집객 및 결제 전환 솔루션:**
          1. **타임 세일/픽업 매대:** 유동 집약 시간대에 '빠른 결제 픽업존' 및 인기 원데이 렌즈 할인 입간판을 전면 노출하세요.
          2. **크로스셀링(Cross-selling):** 결제 단계에서 전용 보습 세척액 또는 렌즈 케이스 세트 할인을 제안하여 건당 결제액을 증대시키세요.
        """,
        "경쟁성·희소성": f"""
        * **경쟁 밀집도 및 상권 독점력:** 반경 300m 이내 동일 업종 안경원, 경쟁 렌즈 브랜드, 뷰티 샵의 밀집 수준과 점유율 방어력을 평가해 **{s_comp}점 / 15점**을 수여했습니다.
        * **차별화 및 승리 전략:** 단순 소모성 가격 할인 경쟁에 매몰될 경우 가맹점 마진율이 훼손되므로, **{store_name}**만의 차별화된 라인업을 앞세워야 합니다.
        * **핵심 경쟁 차별화 솔루션:**
          1. **렌즈미 독점 PB 강화:** 악마원데이, 트루핏 등 일반 안경원에는 없는 렌즈미 전용 PB 라인업을 메인 윈도우에 전면 연출하세요.
          2. **퍼스널 렌즈 가이드:** 눈동자 톤에 맞춘 퍼스널 컬러 렌즈 가이드북을 비치하여 전문 뷰티 렌즈 샵으로서의 브랜딩을 강화하세요.
        """,
        "입지 안정성": f"""
        * **임대료 고정비 비율 & 손익 방어:** 임대료 및 점포 유지비용 대비 매출 창출력, 경기 변동에 대한 상권 방어 탄력성을 다각도로 검토해 **{s_stable}점 / 15점**으로 산정했습니다.
        * **손익분기점(BEP) 관리 방안:** 고정비 부담을 상쇄하고 영업이익률을 극대화하기 위해서는 고마진 상품의 매출 비중 관리가 필수적입니다.
        * **장기 안정성 확보 솔루션:**
          1. **고마진 PB 점유율 확대:** 원가율이 낮고 마진율이 뛰어난 PB 컬러렌즈 판매 비중을 매장 전체 매출의 **40% 이상**으로 유지하세요.
          2. **단골 고객 CRM 자동화:** 방문 고객의 도수 및 구매 주기를 데이터화하여 렌즈 교체 시점에 맞춘 자동 감사 쿠폰 문자를 발송하세요.
        """
    }
