...
# 🧠 [평가 사유 + 해결 방안 1:1 맞춤 제시] 정밀 상권 컨설팅 UI 함수
def render_location_consulting_ui(store_name, store_type, address):
    addr_str = str(address)
    
    # 5대 지표별 [세부 분석 사유] 및 [해결 방안/컨설팅 솔루션] 구성
    if any(k in addr_str for k in ['지하', '지하상가', '역사', '역내']):
        ...
        details = {
            "유동성·접근성": """
            * **[세부 평가 사유]** 지하철 역사 연계 동선으로 출퇴근 유동인구 집약도가 매우 우수하나, 지하 통로 구조상 지상 가시성 및 멀리서 보이는 시인성이 제한됩니다.
            * **[해결 방안 및 컨설팅]** 
              1. **전면 LED 조명 매대 구축:** 매장 입구 전면에 조명이 적용된 PB/컬러렌즈 픽업 진열대를 배치하여 보행 고객의 시선을 유도하세요.
              2. **시선 유도형 VMD 연출:** 출퇴근 이동 동선 방향으로 베스트셀러 렌즈 연출물과 대형 입간판을 전면 배치하세요.
            """,
            "배후 수요성": """
            * **[세부 평가 사유]** 통행 유동인구 절대량은 대규모이나, 반경 500m 내 거주 세대수가 부족해 목적성 재방문보다는 동선 상의 충동 구매 비중이 높습니다.
            * **[해결 방안 및 컨설팅]** 
              1. **단품 및 회전율 중심 구성:** 1만~2만원대 가성비 PB 컬러렌즈 및 원데이 소포장 제품 선반을 확충하여 즉각적인 결제를 유도하세요.
              2. **모바일 쿠폰 연동 CRM:** 충동 방문객의 재방문을 위해 첫 결제 시 카카오톡 채널 추가 쿠폰을 제공하여 단골화하세요.
            """,
            "집객력·활성화": """
            * **[세부 평가 사유]** 발자국(Foot Traffic)이 매우 높으나, 퇴근 시간대(17시~20시)에 결제가 집중되는 피크타임 쏠림 현상이 존재합니다.
            * **[해결 방안 및 컨설팅]** 
              1. **퇴근길 타임 세일 픽업존:** 피크 타임 전용 빠른 결제 매대를 운영하여 결제 대기 시간을 줄이고 전환율을 극대화하세요.
              2. **오후 비수기 이벤트:** 낮 시간대 방문객 유입을 위해 부대용품 증정 및 원데이 렌즈 번들 행사 마케팅을 진행하세요.
            """,
            "경쟁성·희소성": """
            * **[세부 평가 사유]** 지하상가 구역 내 저가 안경원 및 렌즈 전문점 밀집도가 높아 가격 경쟁 압박이 존재합니다.
            * **[해결 방안 및 컨설팅]** 
              1. **렌즈미 독점 PB 라인업 전면 배치:** 일반 안경원과 차별화되는 악마원데이, 트루핏 등 독점 렌즈를 메인에 배치하세요.
              2. **퍼스널 컬러 스타일링 제공:** 가격 할인전 대신 개인별 눈동자 톤에 맞는 렌즈 추천 가이드를 제시하여 전문성을 어필하세요.
            """,
            "입지 안정성": """
            * **[세부 평가 사유]** 역사/지하상가 시설 주체와의 재계약 주기 리스크와 높게 형성된 임대료 비율이 고정비 부담 요인입니다.
            * **[해결 방안 및 컨설팅]** 
              1. **고마진 구조 체질 개선:** 원가율이 낮고 마진율이 높은 PB 컬러렌즈 매출 비중을 전체의 45% 이상으로 상향 유지하세요.
              2. **재고 회전율 극대화:** 비인기 도수/컬러 재고를 신속히 정산하고 베스트셀러 중심의 슬림 재고 구조를 확립하세요.
            """
        }
        ...
        details = {
            "유동성·접근성": """
            * **[세부 평가 사유]** 메인 대로변 입지로 보행 유동과 대중교통 접근성, 파사드 전면 간판 가시성이 최고 수준입니다.
            * **[해결 방안 및 컨설팅]** 
              1. **윈도우 디스플레이 강화:** 매장 전면 쇼윈도에 시즌별 메인 테마 렌즈 포스터와 조명 진열을 적용하여 시선을 집중시키세요.
              2. **랜드마크화 연출:** 멀리서도 렌즈미 매장임을 알 수 있는 브랜딩 조명 요소를 적극 활용하세요.
            """,
            "배후 수요성": """
            * **[세부 평가 사유]** 인근 오피스 직장인과 주말 외부 유입 인구가 융합되어 있으나, 소비 연령대가 다변화되어 단일 마케팅이 어렵습니다.
            * **[해결 방안 및 컨설팅]** 
              1. **듀얼 품목 믹스 구성:** 평일은 2030 직장인용 원데이 투명/자연스러운 컬러렌즈, 주말은 화려한 트렌디 렌즈 위주로 소셀링하세요.
              2. **프리미엄 세트 판매:** 고소득 직장인을 타깃으로 원데이 대용량 및 난시용 렌즈의 세트 구성을 제안하세요.
            """,
            "집객력·활성화": """
            * **[세부 평가 사유]** 주중/주말 매출 기복 없이 높은 집객력을 보이나, 매장 내 체류 시간이 짧아 단품 구매에 머무는 경향이 있습니다.
            * **[해결 방안 및 컨설팅]** 
              1. **체험형 렌즈 갤러리 탭 구성:** 고객이 직접 거울을 보며 착용 샷을 비교할 수 있는 시착/체험존을 구축해 체류 시간을 늘리세요.
              2. **크로스 셀링(Cross-selling) 유도:** 렌즈 구매 시 보습 인공눈물 또는 전용 세척액 세트 할인을 제안하세요.
            """,
            "경쟁성·희소성": """
            * **[세부 평가 사유]** 핵심 역세권 특성상 반경 300m 내 대형 경쟁 매장이 밀집하여 브랜드 간 유입 고객 뺏기 경쟁이 치열합니다.
            * **[해결 방안 및 컨설팅]** 
              1. **맞춤형 검안 전문성 강조:** 정밀 검안 장비를 활용한 시력 측정 서비스를 강조하여 신뢰도를 높이세요.
              2. **멤버십 회원 단독 혜택:** 경쟁점 이탈을 막기 위해 가맹점 전용 로열티 포인트 및 등급별 쿠폰을 지급하세요.
            """,
            "입지 안정성": """
            * **[세부 평가 사유]** 집객력은 보장되나 중심 상권 특유의 고임대료 구조로 인해 매출 대비 손익분기점(BEP) 기준선이 높습니다.
            * **[해결 방안 및 컨설팅]** 
              1. **평균 객단가(ATV) 상향:** 1회 방문당 결제 금액을 높일 수 있도록 원데이 멀티팩 할인 및 연간권 프로모션을 도입하세요.
              2. **고마진 PB 비중 관리:** 마진율이 높은 PB 라인업 매출 비중을 끌어올려 임대료 고정비율을 안정적으로 낮추세요.
            """
        }
        ...
        details = {
            "유동성·접근성": """
            * **[세부 평가 사유]** 대학가 메인 로드샵 및 등하교 동선에 위치하여 청년층 보행 유동 접근성은 훌륭하나, 보행 속도가 빨라 지나치기 쉽습니다.
            * **[해결 방안 및 컨설팅]** 
              1. **개방형 팝업 매대 설치:** 입구 근처에 학생들의 발걸음을 멈추게 할 신상품 팝업 거울 매대를 설치하세요.
              2. **트렌디 포토존 구성:** 매장 내/외부에 SNS 인증샷을 찍을 수 있는 조명 요소를 배치하세요.
            """,
            "배후 수요성": """
            * **[세부 평가 사유]** 1020세대 핵심 소비층 밀도가 압도적이지만, 가성비 제품(1만~2만원대) 선호도가 높아 평균 객단가가 낮게 형성됩니다.
            * **[해결 방안 및 컨설팅]** 
              1. **1+1 / 1+2 팩키징 전략:** 가성비를 중시하는 학생들을 위해 팩 렌즈 교차 증정 프로모션을 적극 활용하세요.
              2. **학생증 및 제휴 할인:** 인근 대학 총학생회 제휴 프로모션으로 신입생 유입을 독점하세요.
            """,
            "집객력·활성화": """
            * **[세부 평가 사유]** 개강 시즌(3~6월, 9~12월) 집객력은 폭발적이나, 여름/겨울 방학 기간(연 약 4개월) 동안 매출이 급감합니다.
            * **[해결 방안 및 컨설팅]** 
              1. **학기 중 고객 DB 확보:** 학기 중 방문 고객의 연락처/카카오 채널 등록을 유도하여 방학 중 택배 배송 서비스를 제공하세요.
              2. **종강 직전 대용량 기획전:** 방학 고향 방문 전 미리 구매하도록 팩 렌즈 대용량 특가전을 진행하세요.
            """,
            "경쟁성·희소성": """
            * **[세부 평가 사유]** 대학가 내 유사 저가 컬러렌즈 브랜드와 트렌드/가격 경쟁이 공존합니다.
            * **[해결 방안 및 컨설팅]** 
              1. **SNS 이슈 렌즈 즉시 스팟 진열:** 숏폼/인스타그램에서 화제가 되는 컬러 렌즈를 가장 전면에 즉시 진열하세요.
              2. **인플루언서 픽(Pick) 홍보:** 대학생 선호 스타일리시 렌즈 라인업을 강조하는 연출물을 활용하세요.
            """,
            "입지 안정성": """
            * **[세부 평가 사유]** 유행 주기가 빠른 영타겟 상권 특성상 기존 인기 상품의 수명이 짧아 매출 변동 리스크가 존재합니다.
            * **[해결 방안 및 컨설팅]** 
              1. **신상품 상시 순환 VMD:** 최소 월 1회 매장 메인 디스플레이 컨셉을 교체하여 신선함을 유지하세요.
              2. **비수기 고정비 방어:** 학기 중 축적된 마진으로 방학 비수기 고정비를 충당할 수 있도록 연간 예산을 배분하세요.
            """
        }
    else:
        ...
        details = {
            "유동성·접근성": """
            * **[세부 평가 사유]** 빠른 통행성 유동인구보다는 거주민의 정체형 도보 유동 위주로, 주말/야간 보행량이 다소 적습니다.
            * **[해결 방안 및 컨설팅]** 
              1. **생활 밀착형 홍보:** 아파트 단지 입구, 학원가 동선에 맞춰 검안 및 정기 교체 안내 홍보물을 배치하세요.
              2. **주차 편의성 안내:** 차량 이용 고객이 편하게 방문할 수 있도록 주차 등록 및 건물 주차 안내를 명확히 하세요.
            """,
            "배후 수요성": """
            * **[세부 평가 사유]** 반경 500m 내 대단지 아파트 및 거주민 고정 배후 수요가 매우 견고하며, 가족 단위 구매 고객 비중이 높습니다.
            * **[해결 방안 및 컨설팅]** 
              1. **가족 패키지 제품군 확충:** 학생 자녀용 컬러렌즈와 학부모용 원데이 투명/난시용 렌즈를 묶은 패키지 제안을 강화하세요.
              2. **정기 구독 및 교체 알림 CRM:** 거주민 고객의 렌즈 교체 주기에 맞춰 사전 안내 문자 및 할인 쿠폰을 발송하세요.
            """,
            "집객력·활성화": """
            * **[세부 평가 사유]** 즉흥 구매 비율은 낮으나 정기 시력 검안 및 교체를 위한 단골 목적 방문 비중이 높아 재방문율이 일정합니다.
            * **[해결 방안 및 컨설팅]** 
              1. **정밀 시력 검안 존 강화:** 단순 판매점을 넘어 전문 눈 건강 안경원으로서의 신뢰도를 제공하여 고객 이탈을 방지하세요.
              2. **대용량 투명/난시 렌즈 적극 권유:** 단골 고객 방문 시 마진율과 객단가가 높은 대용량 투명/난시 렌즈 세트 판매를 제안하세요.
            """,
            "경쟁성·희소성": """
            * **[세부 평가 사유]** 주거 밀착형 상권 특성상 신규 경쟁점의 유입 가능성이 낮아 점유율을 독점하기에 용이합니다.
            * **[해결 방안 및 컨설팅]** 
              1. **지역 커뮤니티 마케팅:** 맘카페, 당근마켓 동네생활 등 지역 커뮤니티 이벤트를 활용해 이사 온 신규 주민을 선점하세요.
              2. **학원가 연계 이벤트:** 인근 중고교 학생 대상 시력 검안 캠페인을 추진하여 학생 고객을 확보하세요.
            """,
            "입지 안정성": """
            * **[세부 평가 사유]** 경기 변동이나 외부 리스크에 영향을 적게 받는 배후 주거지로, 지속적이고 안정적인 장기 매출 유지가 가능합니다.
            * **[해결 방안 및 컨설팅]** 
              1. **단골 VIP 제도 운영:** 충성 고객에게 정기적인 무료 부대용품 증정 및 케어 서비스를 제공하여 평생 고객화하세요.
              2. **고마진 PB 비율 확대:** 안정된 매출 기반 위에서 PB 라인업을 강화하여 점포 순이익율을 상향시키세요.
            """
        }
    ...
    st.markdown("<p style='font-size:13px; font-weight:bold; color:#334155; margin-bottom:8px;'>👇 아래 항목 버튼을 누르시면 항목별 세부 평가 사유와 1:1 맞춤형 컨설팅 솔루션을 확인하실 수 있습니다.</p>", unsafe_allow_html=True)
    ...
    # 순수 마크다운 방식으로 노출되는 1:1 평가 및 솔루션 카드
    active_item = st.session_state[session_key]
    st.markdown(f"#### 📌 [{active_item}] 세부 분석 사유 및 맞춤형 컨설팅 솔루션 (점수: {scores[active_item]})")
    ...
    if "지하상가" in location_type or "초역세권" in location_type:
        ...
        st.markdown("💡 **[핵심 실행 전략]** 고정 재방문 단골 비중이 높은 안정적 상권입니다. **원데이 투명렌즈 및 난시용/프리미엄 렌즈 세트 세일즈**와 검안 연계 마케팅으로 평균 객단가를 극대화하세요.")

    if store_type in ["샵앤샵", "아이웨어샵"]:
        st.markdown("👓 **[형태별 컨설팅]** 안경원 병행 매장의 이점을 살려 **근시/난시 정밀 시력검안 서비스**와 안경/렌즈 교차 구매 혜택을 제공하여 시너지를 창출하세요.")
